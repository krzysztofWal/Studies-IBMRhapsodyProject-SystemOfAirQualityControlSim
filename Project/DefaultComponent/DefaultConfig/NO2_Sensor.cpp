...
void NO2_Sensor::rootState_entDef() {
    {
        ...
        NOTIFY_STATE_ENTERED("ROOT.Oczekiwanie_no2");
        rootState_subState = Oczekiwanie_no2;
        rootState_active = Oczekiwanie_no2;
...
IOxfReactive::TakeEventStatus NO2_Sensor::rootState_processEvent() {
    ...
    switch (rootState_active) {
        // State Oczekiwanie_no2
        case Oczekiwanie_no2:
        {
            if(IS_EVENT_TYPE_OF(readNO2_Default_id))
                {
                    NOTIFY_TRANSITION_STARTED("1");
                    NOTIFY_STATE_EXITED("ROOT.Oczekiwanie_no2");
                    //#[ transition 1 
                    odczytajDane();
                    //#]
...
void OMAnimatedNO2_Sensor::rootState_serializeStates(AOMSState* aomsState) const {
    ...
    switch (myReal->rootState_subState) {
        case NO2_Sensor::Oczekiwanie_no2:
        {
            Oczekiwanie_no2_serializeStates(aomsState);
        }
        break;
...
void OMAnimatedNO2_Sensor::Oczekiwanie_no2_serializeStates(AOMSState* aomsState) const {
    aomsState->addState("ROOT.Oczekiwanie_no2");
}
//#]
